...
3. 개인 및 기관의 데이터 축적량 감소 (O)
4. 소셜 미디어의 발전

**02 데이터 과학(data science) 에 관한 설명으로 거리가 먼 것은?**

1. 비정형 데이터만을 분석 대상으로 한다. (O)
...
4. 특별한 사전 지식 없이도 분석 대상 개체들 사이의 유사성 (similarity)에 기초하여 몇 개의 군집으로 그룹화하는 분석 방법이다. (O)

**14 오피니언 마이닝 (opinion mining)에 관한 설명으로 옳은 것은?**

1. 사회 연결망(social network)을 분석하여 연결된 사람들 간의 인간관계의 특성과 구조를 분석하고시각화한다.
2. 어떤 사안이나 이슈, 이벤트, 제품 및 서비스 등에 대해 긍정적인지 부정적인지 분석할 수 있어 감성 분석아라고도 한다. (O)
...
SQL은 정형 데이터를 위한 관계형 데이터베이스로, 정해진 스키마와 ACID 트랜잭션을 보장함.
NoSQL은 비정형/반정형 데이터를 유연하게 처리하며, 수평 확장성과 빠른 처리 속도에 유리함.
SQL은 복잡한 쿼리에, NoSQL은 대용량 분산 처리 및 유연성이 필요한 환경에 적합함.
