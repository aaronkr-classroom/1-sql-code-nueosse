...
1. SELECT 이름 FROM 학생 WHERE 학년 IN (1, 2, 3, 4) ; (o)
...
4. SELECT * FROM 학생 WHERE 학년 IS NULL ; (o)
...
4. 전체학생의 성적이 점수순(내림차순)으로 정렬된다. (o)
...
2. SQL은 대화식 그리고 (배치 처리)의 2가지 방식으로 사용할 수 있다.
...
11. 중첩 질의문 중에서 내부 질의의 WHERE절 검색조건식이 외부 질의에 선언된 테이블의 일부 열을 참조하는 질의를 (상관 쿼리)이라고 한다.
12. FROM절에 명세한 조인 대상 테이불이 n개이면 (n-1)개의 조인 조건식이 필요하다.

**11 다음 학생 테이블에 대해 "1 학년부터 4학년 순으로 학년별 평균 성적을 검색하라.’’는 SQL문을 작성한 것이다. (_____) 안에 알맞은 용어를 쓰시오.**
...
**12 다음 학생 테이블에 대해 “이 씨 성을 가진 2,3,4학년 학생들의 성적을 내림차순으로 검색하라 .’’는 SQL문을 작성한 것이다 (_____) 안에 알맞은 용어를 쓰시오.**
...
**13 다음 과목 테이블에 내해 ‘‘ 1 학년을 제외한 2,3,4학년별 평균성적이 80점 이상인 과목에 내해서 과목별 학생수를 검색하라.’’는 SQL문을 작성한 것이다. (_____) 안에 알맞은 용어를 쓰시오.**
...
**15 다음 ‘학생’ 테이블에 대해 4학년 학생들의 성적을 10점씩 증가시키는 SQL 명령문율 쓰시오.**
